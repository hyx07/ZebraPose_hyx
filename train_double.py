...
class Trainer():
    def __init__(self, zebra_net, optimizer, model_wrapper, train_dataloader, train_dataloader2, obj_info, test_dataloader, writer, opt):
        ...
        self.train_dataloader2_iter = iter(self.train_dataloader2)
    ...
    def run_epoch(self, epoch):
        ...
        for batch_idx, (data, entire_masks, obj_masks, Rs, ts, Bboxes, class_code_images, cam_Ks) in enumerate(self.train_dataloader):
            inputs1, masks1, binary_codes1 = data, obj_masks, class_code_images
            try:
                data_2, entire_masks_2, obj_masks_2, Rs_2, ts_2, Bboxes_2, class_code_images_2, cam_Ks_2 = next(self.train_dataloader2_iter)
            except StopIteration:
                self.train_dataloader2_iter = iter(self.train_dataloder2)
                data_2, entire_masks_2, obj_masks_2, Rs_2, ts_2, Bboxes_2, class_code_images_2, cam_Ks_2 = next(self.train_dataloader2_iter)
            inputs2, masks2, binary_codes2 = data_2, obj_masks_2, class_code_images_2

            inputs = torch.cat((inputs1, inputs2), 0)
            masks = torch.cat((masks1, masks2),0)
            binary_codes = torch.cat((binary_codes1, binary_codes2),0)
            Rs = torch.cat((Rs, Rs_2),0)
            ts = torch.cat((ts, ts_2),0)
            Bboxes = torch.cat((Bboxes, Bboxes_2),0)
            cam_Ks = torch.cat((cam_Ks, cam_Ks_2),0)
            
            loss = self.run_batch(inputs, masks, binary_codes)
            ...
            iter_num = batch_idx + (epoch-1)*len(self.train_dataloader)
            if (iter_num+1)%self.opt.train_ADD_iter_interval == 0:
                train_ADD_passed, train_ADD_error, train_AUC_ADD_error = evaluate_train_model(self.model_wrapper, self.zebra_net, self.obj_info, self.opt, inputs, masks, Rs, ts, Bboxes, cam_Ks, 0, self.gpu_id)
...
def load_train_obj(opt):
    ...
    batch_size2 = int(opt.batch_size*opt.second_dataset_ratio)
    train_dataloader2, _ = get_train_dataloader(opt, opt.training_data_folder2, batch_size2)
    ...
    return zebra_net, optimizer, model_wrapper, train_dataloader, train_dataloader2, obj_info, test_dataloader, writer
...
def main(opt):
    ddp_setup()
    zebra_net, optimizer, model_wrapper, train_dataloader, train_dataloader2, obj_info, test_dataloader, writer = load_train_obj(opt)
    trainer = Trainer(zebra_net, optimizer, model_wrapper, train_dataloader, train_dataloader2, obj_info, test_dataloader, writer, opt)
    trainer.train()
...
if __name__ == '__main__':
    ...
    opt.training_data_folder2 = 'data folder2 name in dataset folder'
