...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/he/projects/6D_pose_estimation/ZebraPose_parallel/other/pretrained_models/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers, concat_decoder):
        super(ResNet_BasicBlock_OS8, self).__init__()
        self.concat_decoder = concat_decoder
        if num_layers == 18:
            resnet = models.resnet18()
            # load pretrained model:
            resnet.load_state_dict(torch.load("/root/deeplabv3/pretrained_models/resnet/resnet18-5c106cde.pth"))
            ...
            filename = os.path.join(dirname, '/home/he/projects/6D_pose_estimation/ZebraPose_parallel/other/pretrained_models/resnet34-333f7ec4.pth')
            resnet.load_state_dict(torch.load(filename))
            # remove fully connected layer, avg pool, layer4 and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-4])

            if concat_decoder == True:
                # first conv, bn, relu
            ...
            # load pretrained model:
            dirname = os.path.dirname(__file__)
            filename = os.path.join(dirname, '../pretrained_backbone/resnet/resnet50-19c8e357.pth')
            resnet.load_state_dict(torch.load(filename))
            # remove fully connected layer, avg pool, layer4 and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-4])
            ...
            num_blocks_layer_4 = 6
            num_blocks_layer_5 = 3
            self.layer4 = make_layer(BasicBlock, in_channels=512, channels=1024, num_blocks=num_blocks_layer_4, stride=1, dilation=2)
            self.layer5 = make_layer(BasicBlock, in_channels=1024, channels=2048, num_blocks=num_blocks_layer_5, stride=1, dilation=4)
            print ("pretrained resnet, 50")
        else:
            raise Exception("num_layers must be in {18, 34,50}!")
...
def ResNet34_OS8(num_layers=34, concat_decoder=True):
    return ResNet_BasicBlock_OS8(num_layers=num_layers, concat_decoder=concat_decoder)

def ResNet50_OS8(num_layers=50, concat_decoder=True):
    return ResNet_BasicBlock_OS8(num_layers=num_layers, concat_decoder=concat_decoder)
