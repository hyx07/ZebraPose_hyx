...
import pickle
import sys
from .class_id_encoder_decoder import RGB_image_to_class_id_image, class_id_image_to_class_code_images
...
class bop_dataset_single_obj_pytorch_dataset(Dataset):
    def __init__(self, opt, dataset_dir, data_folder, rgb_files, mask_files, mask_visib_files, gts, gt_infos, cam_params, 
        ...
        if self.is_train:
            self.gt_img_code_dir = opt.bop_path + opt.dataset_name + '/GT_img_code/' + data_folder + '/' + opt.obj_name + '/'
        else:
            self.gt_img_code_dir = opt.bop_path + opt.dataset_name + '/GT_img_code/' + opt.val_folder + '/'+ opt.obj_name+'/'
    ...
    def __getitem__(self, index):
        ...
        #print('mask_visib_fns:', mask_visib_fns)
        mask_fns = self.mask_files[index]

        full_rgb = cv2.imread(rgb_fn)
...
##      GT_img_fn = os.path.join(GT_img_dir, GT_image_name)        
        ...
        #GT_img = np.zeros((480,640), dtype=np.uint8)

        GT_img_fn = self.gt_img_code_dir + scene_id + '/' + GT_image_name
        GT_img = cv2.imread(GT_img_fn, cv2.IMREAD_UNCHANGED)
        ...
        #print(Bbox)
        if self.is_train:           
            x = self.apply_augmentation(full_rgb)
            ...
            if not os.path.exists(GT_img_fn):
                ...
                GT_img = np.zeros((full_rgb.shape[0], full_rgb.shape[1]))
                mask = np.zeros((full_rgb.shape[0], full_rgb.shape[1]))
                entire_mask = np.zeros((full_rgb.shape[0], full_rgb.shape[1]))

            Bbox = padding_Bbox(Bbox, padding_ratio=self.padding_ratio)
            roi_x = get_roi(full_rgb, Bbox, self.crop_size_img, interpolation=cv2.INTER_LINEAR, resize_method = self.resize_method)
            roi_GT_img = get_roi(GT_img, Bbox, self.crop_size_gt, interpolation=cv2.INTER_NEAREST, resize_method = self.resize_method)
            roi_mask = get_roi(mask, Bbox, self.crop_size_gt, interpolation=cv2.INTER_NEAREST, resize_method = self.resize_method)
            roi_entire_mask = get_roi(entire_mask, Bbox, self.crop_size_gt, interpolation=cv2.INTER_NEAREST, resize_method = self.resize_method)
            
            #print("show test_image")
            Bbox = get_final_Bbox(Bbox, self.resize_method, full_rgb.shape[1], full_rgb.shape[0])
            # self.visulize(roi_x, roi_entire_mask, roi_mask, roi_GT_img_visible, roi_GT_img_invisible, None)

        roi_GT_img = class_id_image_to_class_code_images(roi_GT_img, self.GT_code_infos[0], self.GT_code_infos[1], self.GT_code_infos[2])
        ...
        return full_rgb, roi_x, roi_entire_mask, roi_mask, R, t, Bbox, class_code_images, cam_param
    ...
    def apply_augmentation(self, x):
        augmentations = build_augmentations(self.use_peper_salt, self.use_motion_blur)      
        color_aug_prob = 0.8
